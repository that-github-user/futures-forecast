/**
 * DesktopTerminalChartCanvas — ECharts-driven candlestick + overlays.
 *
 * Loaded via the lazy-import boundary in `TerminalChartCanvas.tsx`
 * for >768px viewports only. Mobile viewports route to
 * `MobileChartCanvas.tsx` (Lightweight Charts).
 *
 * Polls /terminal/v1/bars/es-intraday every 30s and renders a
 * candlestick chart of the last ~48h of ES bars (RTH + ETH/Globex).
 * The AVWAP system supports three independent anchors {Week, Daily
 * Globex, RTH} × three rendering options {VWAP line, ±1σ band,
 * ±2σ band}. Static markLine overlays for POC/VAH/VAL, prior-day
 * HLC, opening range are driven by the caller-supplied `overlays`
 * toggle state.
 *
...
// Type definitions for OverlayState / Timeframe / VwapAnchorState etc.
// live in `./chartTypes` so the desktop's heavy ECharts imports
// don't pull into the type-import path used by mobile + parent
// dashboard. Imported above.
...
export function DesktopTerminalChartCanvas({
  snapshot,
  overlays,
  timeframe,
  formatBarTime,
